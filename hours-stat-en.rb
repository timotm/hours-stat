...
months = %w(January February March April May June July August September October November December).map { |m| "#{m}" }
...
class ProjectStore
  ...
  def billable(tuntikoodi)
    ...
    if not @projects.has_key? project
      $stderr.puts "*** Unknown project #{project}, assuming it's non-billable".red
    end
...
#    kuussa_tunteja_yhteensä = 7.5 * workdays;
    kuussa_tunteja_yhteensä = 8 * workdays;
    ...
    puts "\n  ### #{months[month-1]} #{year} (#{business_days} business days, #{holidays} holidays and #{workdays} work days)"

    puts "  Total of #{tehdyt_tunnit_yhteensä} h out of #{kuussa_tunteja_yhteensä} of the month, out of which"
    puts "    - billable #{laskutettavat_yhteensä} h, billing ratio #{perce(div(laskutettavat_yhteensä, kuussa_tunteja_yhteensä))} %"
    laskutettavat.nonzero_values_descending.each do |tuntikoodi, tunnit|
      puts "       #{tunnit}\t#{tuntikoodi}"
    end

     puts "    - non-billable #{muut_yhteensä} h, non-billing ratio #{perce(div(muut_yhteensä, kuussa_tunteja_yhteensä))} %"
     muut.nonzero_values_descending.each do |tuntikoodi, tunnit|
       puts "       #{tunnit}\t#{tuntikoodi}"
    end
  end

  puts "\nYear #{year} total:\n#{koko_vuoden_tehdyt} h out of #{vuodessa_tunteja} hours of the year, out of which #{koko_vuoden_laskutettavat} h billable, billing ratio #{perce(div(koko_vuoden_laskutettavat, vuodessa_tunteja))} %\n\n"
end
