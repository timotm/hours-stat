...
OptionParser.new do |opt|
  ...
  opt.on("--server SERVER", String, "Server, server.here") { |s| $options[:server] = s.gsub(/^.*:\/\//, "") }
end.parse!

abort("ERROR: Missing session cookie") unless $options.has_key?(:session)
abort("ERROR: Missing server url") unless $options.has_key?(:server)
...
def billable?(code, billable_codes)
  ...
  warn format(t(:unknown_project), project: project) unless billable_codes.has_key? project
...
expectedHours = summaries.map { |k, v| [k, v["requiredHours"]] }.to_h
...
summary_by_month = code_hour_pairs_by_month.map do |month, hours|
  [month, hours.reduce(Hash.new(0.0)) do |agg, e|
    agg[e[0]] += e[1].to_f
    agg
  end]
end.to_h

months_by_year = summary_by_month.reduce({}) do |agg, (yymm, value)|
  (y, m) = yymm.split("-")
  agg[y] = {} unless agg.has_key?(y)
  agg[y][m.to_i] = value
  agg
end
...
months_by_year.sort_by { |k, _| k }.each do |year, codes_hours_by_month|
  ...
  codes_hours_by_month.sort_by { |k, _| k }.each do |month, hours_by_code|
    ...
    puts format(t(:month_total), done_hours: all_hours_total, hours_in_month: hours_in_this_month)
